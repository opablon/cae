...
const latentDim1ValueSpan = document.getElementById('latentDim1Value'); // Este span es para el valor numérico al lado del slider
const latentDim2ValueSpan = document.getElementById('latentDim2Value'); // Este span es para el valor numérico al lado del slider
const resetViewButton = document.getElementById('resetView');
const inputLatentX = document.getElementById('inputLatentX'); // Nuevo input editable para X
const inputLatentY = document.getElementById('inputLatentY'); // Nuevo input editable para Y
...
const plotMargin = 60;   // Margen alrededor del gráfico para asegurar que los puntos no queden en el borde (AUMENTADO)
...
async function loadLatentData() {
    try {
        ...
        generateLetter([0, 0]); // Generar la letra inicial después de cargar todo
    } catch (error) {
...
function drawGridAndAxes(width, height, xMin, xMax, yMin, yMax) {
    ...
    for (let i = 0; i <= 5; i++) {
        ...
        plotCtx.fillText(val.toFixed(1), xPixel, height - plotMargin + 15); // Ajustado para más margen
    }
    ...
    for (let i = 0; i <= 5; i++) {
        ...
        plotCtx.fillText(val.toFixed(1), plotMargin - 15, yPixel); // Ajustado para más margen
    }
    ...
    plotCtx.fillText('Dimensión Latente 1', width / 2, height - plotMargin + 40); // Ajustado para más margen

    plotCtx.save(); // Guarda el estado actual del contexto
    plotCtx.translate(plotMargin - 45, height / 2); // Mueve el origen para rotar (AJUSTADO)
...
function drawLatentSpace() {
    ...
    const xScale = (width - 2 * plotMargin) / (xMaxGlobal - xMinGlobal);
    const yScale = (height - 2 * plotMargin) / (yMaxGlobal - yMinGlobal);

    // Dibujar cada punto y su etiqueta
    plotData.forEach(point => {
        // Convertir coordenadas latentes a coordenadas de píxeles en el canvas
        const xPixel = plotMargin + (point.x - xMinGlobal) * xScale;
        // El eje Y del canvas va de arriba (0) a abajo (max), así que lo invertimos para el gráfico
        const yPixel = height - plotMargin - (point.y - yMinGlobal) * yScale;
...
plotCanvas.addEventListener('click', async (event) => {
    ...
    if (xPixel < plotMargin || xPixel > (width - plotMargin) ||
        yPixel < plotMargin || yPixel > (height - plotMargin)) {
    ...
    const latentX = xMinGlobal + (xPixel - plotMargin) / xScale;
    const latentY = yMaxGlobal - (yPixel - plotMargin) / yScale; // Invertir el eje Y de nuevo para obtener el valor latente correcto
