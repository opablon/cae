...
const inputLatentX = document.getElementById('inputLatentX');
const inputLatentY = document.getElementById('inputLatentY');
...
const plotMargin = 60;   // Margen alrededor del gráfico para asegurar que los puntos no queden en el borde
...
async function loadModel() {
    try {
        ...
        // Solo generamos si los datos latentes ya se cargaron y los sliders se ajustaron.
        // Opcional: generateLetter([0, 0]); si quieres una letra por defecto antes de que todo esté listo
    } catch (error) {
...
async function loadLatentData() {
    try {
        ...
        // O al punto (0,0) si es el centro del espacio latente para tu modelo
...
function drawGridAndAxes(width, height, xMin, xMax, yMin, yMax) {
    ...
    plotCtx.translate(plotMargin - 45, height / 2); // Mueve el origen para rotar
...
latentDim1Slider.addEventListener('input', () => {
    ...
    inputLatentX.value = value.toFixed(2);
...
latentDim2Slider.addEventListener('input', () => {
    ...
    inputLatentY.value = value.toFixed(2);
...
resetViewButton.addEventListener('click', () => {
    ...
    latentDim2ValueSpan.textContent = '0.00';
    currentLatentYSpan.textContent = '0.00';
    inputLatentX.value = '0.00';
    inputLatentY.value = '0.00';
